...
static struct process* processes[CIMAOS_MAX_PROCESSES]= {};
...
{
    if(process_id < 0 || process_id >= CIMAOS_MAX_PROCESSES)
    {
        return NULL;
    }
...
int process_map_binary(struct process* process)
{
    ...
    paging_map_to(process->task->page_directory, (void*)CIMAOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITABLE);
...
int process_map_memory(struct process* process)
{
    ...
    paging_map_to(process->task->page_directory, (void*) CIMAOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack+CIMAOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_IS_WRITABLE | PAGING_ACCESS_FROM_ALL);
out:
    return res;
...
{
    for (int i = 0; i < CIMAOS_MAX_PROCESSES; i++)
    {
        if (processes[i] == 0)
            return i;
    }
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(CIMAOS_USER_PROGRAM_STACK_SIZE);

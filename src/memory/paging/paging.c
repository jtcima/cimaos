...
int paging_map_to(uint32_t* directory, void* virt, void* phys_addr, void* phys_addr_end, int flag)
{
    int res = 0;
    ...
    if((uint32_t)phys_addr % PAGING_PAGE_SIZE)
    {
        res = -EINVARG;
        goto out;
    }
    if((uint32_t)phys_addr_end % PAGING_PAGE_SIZE)
    {
        res = -EINVARG;
        goto out;
    }
    if((uint32_t)phys_addr_end < (uint32_t)phys_addr)
    {
        res = -EINVARG;
        goto out;
    }

    uint32_t total_bytes = phys_addr_end - phys_addr;
    int total_pages = total_bytes / PAGING_PAGE_SIZE;
    res = paging_map_range(directory, virt, phys_addr, total_pages, flag);
out:
    return res;
}

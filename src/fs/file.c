...
struct filesystem* filesystems[CIMAOS_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[CIMAOS_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_filesystem()
{
    int i = 0;
    for(i = 0 ; i < CIMAOS_MAX_FILESYSTEMS; i++)
    {
        if (filesystems[i] == 0)
        {
...
{
    int res = -ENOMEM;
    for(int i = 0; i < CIMAOS_MAX_FILE_DESCRIPTORS; i++)
    {
...
{
    if(fd <=0 || fd >= CIMAOS_MAX_FILE_DESCRIPTORS)
    {
        return 0;
    }
...
struct filesystem* fs_resolve(struct disk* disk)
{
    ...
    for(int i = 0; i < CIMAOS_MAX_FILESYSTEMS; i++)
    {
...
int fopen(const char* filename, const char* mode_string)
{
    ...
    //we can not just have a root path like 0:/
    if(!root_path->first)
    {
        res = -EINVARG;
        goto out;
...
out:
    if(res < 0)
        res = 0; //fopen should only return null if it fails not nageitve value
    return res;
}

...
{
    int len = strnlen(filename, CIMAOS_MAX_PATH);
...
static int pathparser_get_drive_by_path(const char** path)
{
    ...
    //add 3 bytes to skip drive number eg. 0:/ 1:/
    *path += 3;
    return drive_no;
}
...
{
    char* result_path_part = kzalloc(CIMAOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    if(strlen(path) > CIMAOS_MAX_PATH)
    {
        goto out;
    }

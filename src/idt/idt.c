...
struct idt_desc idt_descriptors[CIMAOS_TOTAL_INTERRUPTS];

struct idtr_desc idtr_descriptor;

extern void* interrupt_pointer_table[CIMAOS_TOTAL_INTERRUPTS];

static INTERRUPT_CALLBACK_FUNCTION interrupt_callbacks[CIMAOS_TOTAL_INTERRUPTS];
...
int idt_register_interrupt_callback(int interrupt, INTERRUPT_CALLBACK_FUNCTION interrupt_callback)
{
    ...
    if (interrupt < 0 || interrupt >= CIMAOS_TOTAL_INTERRUPTS)
    {
        return -EINVARG;
    }
